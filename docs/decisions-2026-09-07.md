...
### 55. fail the production build when the Web3Forms key is missing, rather than warn

The first live deployment shipped a contact form that posted `access_key: ""` and was rejected for every visitor. Nothing surfaced it: there is no server to log the failure, the visitor saw only the generic error, and the owner saw nothing at all — the message simply never arrived. The variable existed in the Vercel dashboard the whole time; it was scoped to Production while the deployment being served was built as a Preview, and `NEXT_PUBLIC_*` is inlined as literal text at build time rather than read at runtime, so the dashboard value never reached the bundle. Build time is the only moment the check can still do anything, so `lib/env.ts` throws from `next.config.ts` on production builds only, leaving `next dev` alone. The error message names both dashboard causes — environment scoping and the build cache — because it fires on a build host where the reader has no context and cannot inspect anything; a unit test asserts those two phrases survive, since a guard that stops the build without saying why is barely better than the silence it replaces. Cost if wrong: a legitimate build can be blocked by a genuinely absent key, which is the intended trade — a failed build is loud and takes one dashboard change to clear, whereas the alternative is a portfolio whose contact form silently discards messages from recruiters.
