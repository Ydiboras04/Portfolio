...
export const fr: Dictionary = {
  ...
  credibility: [
    ...
    // "européens" is the differentiator (not everyone's clients are European), so it
    // carries the amber -- but French puts the adjective after the noun ("clients
    // européens", never "européens clients"), the reverse of en.ts's "European
    // clients". accentFirst renders the plain `accent` ("Clients ") before the amber
    // `value` ("européens") so the sentence stays "Clients européens" either way.
    { value: 'européens', accent: 'Clients ', accentFirst: true, label: 'Automatisation documentaire livrée depuis Antananarivo' },
